...
IMPLEMENT_SYNTHESIZE_SINGLETON_FOR_CLASS(IAPManager)
    ...
    for (SKPaymentTransaction *transaction in transactions) {
        ...
        switch (transaction.transactionState) {
            ...
            case SKPaymentTransactionStatePurchased://交易完成
                ...
                //交易成功,直接回调
            [self filedWithErrorCode:IAP_SUCESSCODE error:nil];
...
-(void)saveReceipt {
    ...
    self.date = [NSDate getDateFormateStr];
    
    NSString *fileName =[AlUtil getuuid];
   
    
    //存储用户的id
    
    NSString *savedPath = [NSString stringWithFormat:@"%@/%@.plist", [SandBoxHelper iapReceiptPath], fileName];
    NSString *userid=[[ALDef objectForKey:@"appSaveKey"] objectForKey:@"accountKey"];
    NSDictionary *dic =[NSDictionary dictionaryWithObjectsAndKeys:
                        self.receipt,                           receiptKey,
                        self.date,                              dateKey,
                        self.orderId,                            orderIdKey,
                        userid,                            userIdKey,
...
//    NSData *receiptData = [NSData dataWithContentsOfURL:receiptURL];
    ...
    NSMutableDictionary *params = [NSMutableDictionary dictionaryWithObjectsAndKeys:
                                   ...
                                   [dic objectForKey:orderIdKey],           orderIdKey,
...
-(void)reqSer:(NSDictionary *)params{
    NSLog(@"服务器验证---");
//    // 1.根据网址初始化OC字符串对象
//    NSString *urlStr = [NSString stringWithFormat:@"%@", getrecpt_url];
//    // 2.创建NSURL对象
...
//
//    NSString *res=[[NSString alloc] initWithData:resData encoding:NSUTF8StringEncoding];   //将结果转为字符串
...
//    NSDictionary *responseJSON = [NSJSONSerialization JSONObjectWithData:xmlData options:NSJSONReadingMutableLeaves error:nil];
 [self removeReceipt];
//    if(responseJSON){
//        int state=[responseJSON[@"state"] intValue];;
//                if(state==1){
//                    int code=[responseJSON[@"rcode"] intValue];;
//
//
//                    if(code==0){
//
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(filedWithErrorCode:andError:)]) {
        switch (code) {
            ...
            case IAP_FILEDCOED_USERCANCEL:
                ...
                break;
            case IAP_SUCESSCODE:
                [self.delegate filedWithErrorCode:IAP_SUCESSCODE andError:nil];
                break;
            case IAP_INVA:
                [self.delegate filedWithErrorCode:IAP_INVA andError:nil];
                break;
